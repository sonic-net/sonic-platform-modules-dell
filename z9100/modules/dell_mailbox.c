...
#define DRIVER_NAME "dell_mailbox"
...
static struct resource dell_z9100_lpc_resources[] = {
    {
...
static void dell_z9100_lpc_dev_release( struct device * dev)
{
    return ;
}

static struct platform_device dell_z9100_lpc_dev = {
    .name           = DRIVER_NAME,
    .id             = -1,
    .num_resources  = ARRAY_SIZE(dell_z9100_lpc_resources),
    .resource       = dell_z9100_lpc_resources,
    .dev = {
        .release = dell_z9100_lpc_dev_release,
...
static struct attribute *z9100_temperature_attrs[] = {
...
static struct attribute *z9100_fantray_attrs[] = {
...
static struct attribute_group z9100_lpc_attr_grp = {
    .attrs = z9100_lpc_attrs,
};
static struct attribute_group z9100_temperature_attr_grp = {
    .attrs = z9100_temperature_attrs,
};

static struct attribute_group z9100_fantray_attr_grp = {
    .attrs = z9100_fantray_attrs,
};

static struct attribute_group z9100_psu_attr_grp = {
    .attrs = z9100_psu_attrs,
};


static int dell_z9100_lpc_drv_probe(struct platform_device *pdev)
{
    struct resource *res;
    int ret =0;
...
static int dell_z9100_lpc_drv_remove(struct platform_device *pdev)
{

    sysfs_remove_group(&pdev->dev.kobj, &z9100_lpc_attr_grp);

    return 0;
}

static struct platform_driver dell_z9100_lpc_drv = {
    .probe          = dell_z9100_lpc_drv_probe,
    .remove         = __exit_p(dell_z9100_lpc_drv_remove),
    .driver = {
        .name  = DRIVER_NAME,
    },
};
...
void dell_z9100_lpc_exit(void)
{
    platform_device_unregister(&dell_z9100_lpc_dev);
    platform_driver_unregister(&dell_z9100_lpc_drv);
    return;
}

module_init(dell_z9100_lpc_init);
module_exit(dell_z9100_lpc_exit);

MODULE_AUTHOR("Srideep Devireddy  <srideep_devireddy@dell.com>");
MODULE_DESCRIPTION("dell_z9100_lpc driver");
MODULE_LICENSE("GPL");
